...
class HideMyEmail:
    ...
    def __init__(self, label: str = "Xtoun's gen", cookies: str = ""):
        """Initializes the HideMyEmail class.

        Args:
            label (str)     Label that will be set for all emails generated, defaults to `Xtoun's gen`
            cookies (str)   Cookie string to be used with requests. Required for authorization.
        """
        # Label that will be set for all emails generated, defaults to `Xtoun's gen`
        self.label = label
    ...
    async def reserve_email(self, email: str) -> dict:
        ...
        try:
            payload = {
                "hme": email,
                "label": self.label,
                "note": "Generated by Xtoun's iCloud email generator",
            }
            async with self.s.post(
